...
export default class extends Command {
  ...
  async exec(msg: Message, args: string[]) {
    ...
    const userId = args[0];
    if (!userId) return msg.channel.send('You need to specify user id');

    const awardType = args[1];
    if (!awardType) return msg.channel.send('You need to give award type');

    const amount = args[2];
    if (!amount) return msg.channel.send('You need to give valid amount or medal type');
    ...
    if (!isAdmin) return msg.channel.send('Only admin can use this command');
    ...
    if (awardType === 'medal') {
      ...
      if (!amountInt) return msg.channel.send('Please give valid amount');
