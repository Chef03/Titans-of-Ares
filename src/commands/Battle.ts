import {
  Collection, Message, MessageEmbed, MessageReaction, User,
} from 'discord.js';
import { DateTime } from 'luxon';
import { setMaxChallenger } from '../db/challenger';
import {
  hasTimer, setEnergy, setTimer, TimerType,
} from '../db/timer';
...
export default class extends Command {
  ...
  async exec(msg: Message, args: string[]) {
    ...
    try {
      ...
      if (!prevEnergyTimer) await setTimer(TimerType.Energy, player.id, expireDate);
      ...
      const battle = (count: number) => async () => {
        client.safeFn.add(
          safeFnID,
