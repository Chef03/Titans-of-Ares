...
import {
  BLUE_BUTTON, bold, BROWN, capitalize, GREEN, inlineCode, NUMBER_BUTTONS, sleep,
...
export default class Combine extends Command {
  ...
  async exec(msg: Message, args: string[]) {
    ...
    if (indexes.some((x) => Number.isNaN(x))) return msg.channel.send('invalid index was given');
    if (indexes.length === 0) return msg.channel.send('you need to pick gems you want to combine');
    ...
    for (const index of indexes) {
      ...
      if (!gem) { return msg.channel.send(`cannot find gem on index ${index}`); }
