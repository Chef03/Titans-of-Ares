...
export default class extends Command {
  ...
  async exec(msg: Message, args: string[]) {
    ...
    if (index === 'all') {
      ...
      if (Number.isNaN(parseInt(index))) return msg.channel.send('Please give valid number');

      const pet = player.pets.get(parseInt(index) - 1);
      if (!pet) return msg.channel.send('Please give valid index');
      ...
      button.addButton('🔴', 'deactivate current active pet', () => {
        if (!player.activePet) return msg.channel.send('You currently have no active pet');
