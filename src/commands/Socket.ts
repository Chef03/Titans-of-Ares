...
import {
  BLUE_BUTTON, bold, GREEN, inlineCode, RED_BUTTON, WHITE_BUTTON,
...
export default class Socket extends Command {
  ...
  async exec(msg: Message, args: string[]) {
    ...
    if (!Gem.isValidQuality(quality)) return msg.channel.send('invalid Gem quality');
    if (!index) return msg.channel.send('invalid index');
    if (Number.isNaN(index)) return msg.channel.send('invalid index');

    const player = await Player.getPlayer(msg.member!);
    const gems = player.inventory.gems.filter((x) => x.quality === quality);

    if (gems.length <= 0) return msg.channel.send(`You don't have any gem of ${quality} quality`);

    const gemList = List.from(gems).aggregate();
    const selected = gemList[index - 1];

    if (!selected) return msg.channel.send('Cannot find gem');
    ...
    const buttonHandler = async (btn: string | undefined) => {
      ...
      if (!piece) return msg.channel.send('The selected piece must be equipped first');
      if (piece.gem) return msg.channel.send('Selected gear has already gem socketed');
