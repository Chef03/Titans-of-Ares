...
import {
  BROWN, CDN_LINK, GOLD, numberFormat, roundTo, STAR,
} from './utils';
import { Pet as PetDB } from '../db/pet';
import { Player } from './Player';
import { List } from './List';
import { client } from '../main';
import { player } from './SquadBattle';
import { fullPlayer } from './FightBoss';
...
export abstract class Pet {
  ...
  card(fragmentCount: number, showPossession = false) {
    ...
    if (this.star !== -1) {
      ...
      embed.addField(
        'Summoned On',
        this.createdAt.toLocaleString(DateTime.DATE_MED),
        true,
      );
    }
